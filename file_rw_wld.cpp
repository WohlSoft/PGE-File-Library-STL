...
bool FileFormats::ReadSMBX64WldFileHeader(PGESTRING filePath, WorldData &FileData)
{
    ...
    try
    {
        ...
        if(ge(55))
        {
            nextLine(); SMBX64::ReadCSVBool(&FileData.nocharacter1, line);//Edisode without Mario
            nextLine(); SMBX64::ReadCSVBool(&FileData.nocharacter2, line);//Edisode without Luigi
            nextLine(); SMBX64::ReadCSVBool(&FileData.nocharacter3, line);//Edisode without Peach
            nextLine(); SMBX64::ReadCSVBool(&FileData.nocharacter4, line);//Edisode without Toad
            if(ge(56))
            {
                nextLine(); SMBX64::ReadCSVBool(&FileData.nocharacter5, line);//Edisode without Link
            }
        ...
        if(ge(3))
        {
            nextLine(); SMBX64::ReadStr(&FileData.IntroLevel_file, line);//Autostart level
            nextLine(); SMBX64::ReadCSVBool(&FileData.HubStyledWorld,line);//Don't use world map on this episode
            nextLine(); SMBX64::ReadCSVBool(&FileData.restartlevel, line);//Restart level on playable character's death
        }

        if(ge(20))
        {
            nextLine(); SMBX64::ReadUInt(&FileData.stars, line);//Stars number
        }

        if(file_format >= 17)
        {
            nextLine(); SMBX64::ReadStr(&FileData.author1, line); //Author 1
            nextLine(); SMBX64::ReadStr(&FileData.author2, line); //Author 2
            nextLine(); SMBX64::ReadStr(&FileData.author3, line); //Author 3
            nextLine(); SMBX64::ReadStr(&FileData.author4, line); //Author 4
            nextLine(); SMBX64::ReadStr(&FileData.author5, line); //Author 5
...
bool FileFormats::ReadSMBX64WldFile(PGE_FileFormats_misc::TextInput &in, WorldData &FileData)
{
    ...
    try
    {
        ...
        while( (line!="next")  && (!in.eof()) )
        {
            ...
            nextLine(); SMBX64::ReadSIntFromFloat(&lvlitem.y, line);//Level y
            nextLine(); SMBX64::ReadUInt(&lvlitem.id, line);//Level ID
            nextLine(); SMBX64::ReadStr(&lvlitem.lvlfile, line);//Level file
            nextLine(); SMBX64::ReadStr(&lvlitem.title, line);//Level title
            nextLine(); SMBX64::ReadSInt(&lvlitem.top_exit, line);//Top exit
            nextLine(); SMBX64::ReadSInt(&lvlitem.left_exit, line);//Left exit
            nextLine(); SMBX64::ReadSInt(&lvlitem.bottom_exit, line);//bottom exit
            nextLine(); SMBX64::ReadSInt(&lvlitem.right_exit, line);//right exit
            if(ge(4)) { nextLine(); SMBX64::ReadUInt(&lvlitem.entertowarp, line);}//Enter via Level's warp

            if(ge(22))
            {
                nextLine(); SMBX64::ReadCSVBool(&lvlitem.alwaysVisible, line);//Always Visible
                nextLine(); SMBX64::ReadCSVBool(&lvlitem.pathbg, line);//Path background
                nextLine(); SMBX64::ReadCSVBool(&lvlitem.gamestart, line);//Game start point
                nextLine(); SMBX64::ReadSInt(&lvlitem.gotox, line);//Goto x on World map
                nextLine(); SMBX64::ReadSInt(&lvlitem.gotoy, line);//Goto y on World map
                nextLine(); SMBX64::ReadCSVBool(&lvlitem.bigpathbg, line);//Big Path background
            }
            else
            {
                if(lvlitem.id==1)
